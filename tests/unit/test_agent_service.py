...
class TestAgentService:
    ...
    async def test_chat_return_llm_response(self, mock_llm, mock_memory):
        """测试：不使用工具时直接返回答案"""
        ...
        # Act 执行
        result = await service.chat("test_session", "你好")
    ...
    async def test_chat_with_tool_calls(self, mock_llm, mock_memory, mock_tool):
        ...
        # Mock tool.ainvoke 为异步函数（代码实际调用 ainvoke）
        ...
        # Act
        result = await service.chat("test_session", "计算 1+1")

        # Assert
        assert result == "计算结果是 42"
        assert mock_llm.chat_with_tools.call_count == 2  # LLM 被调用了两次
        mock_tool.ainvoke.assert_called_once()  # 工具被调用了一次

    async def test_chat_saves_conversation(self, mock_llm, mock_memory):
        """测试：chat 后应保存对话历史"""
        # Arrange
        response = MagicMock()
        response.tool_calls = []
        ...
        # Act
        await service.chat("test_session", "你好")

        # Assert
        mock_memory.save_conversation.assert_called_once()

    async def test_chat_max_iterations(self, mock_llm, mock_memory):
        """测试：超过最大迭代次数时返回错误提示"""
        # Arrange
        mock_tool = MagicMock()
        ...
        # Act
        result = await service.chat("test_session", "测试")
